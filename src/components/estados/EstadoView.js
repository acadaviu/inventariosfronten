...
import { getEstadosEquipos, createEstadoEquipo, updateEstadoEquipo } from '../../services/estadoService' 
...
const listarEstados = async () => {
    try {
      ...
      Swal.showLoading();
      const respuesta = await getEstadosEquipos();
...
const handleOnChange = (e) => {
  ...
  const handleCrearMarca = async (e) => {
    ...
    try {
      ...
      const respuesta = await createEstadoEquipo(valoresForm)
      ...
      <tbody>
          {
            estados.length > 0 && estados.map((estado, index) => {
              return <tr key={estado._id || index }>
